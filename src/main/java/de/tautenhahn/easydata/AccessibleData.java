...
/**
 * Wraps complete data access and resolves any requests for data elements.<br> First idea was to wrap all data elements
 * into separate access objects of one base type but that looked ugly because the needed types are different after all.
 * <br> Error reporting uses only the given path values, information about where those values came from may be added by
 * the caller.
 *
 * @author TT
 */
public class AccessibleData
{
     ...
     * @return the attribute of specified name, of whatever type. May return null if the attribute does not exist but
     *     parent object does. Accepts constants if surrounded by quotes.
    ...
    public String getString(String attrName)
    {
        ...
        if (result != null && (result instanceof Map || result instanceof List || result.getClass().isArray()))
        {
            throw new ResolverException(
                "expected String but " + attrName + " is of type " + result.getClass().getName());
        }
    ...
    /**
     * Returns a list of sorted elements. This method calls {@link #get(String, Object)} too frequently, optimize in
    ...
    /**
     * Undoes {@link #define(String, Object)}. The value associated by the key before last define call (if any) is
     * restored.
     *
    ...
    private Object getAttribute(Object element, String first)
    {
        ...
        {
            throw new ResolverException(
                "No property '" + first + "' supported for element of type " + element.getClass().getName(), e);
        }
    }
    ...
    private int getIndex(String index, int length)
    {
        ...
        throw new ResolverException("Invalid index '" + index + "', addressed object has elements 0-" + (length - 1));
    }
    ...
    private Collection<Object> beanToList(String attrName, ListMode mode, Object target)
    {
        ...
        {
            throw new ResolverException(
                "expected complex object but " + attrName + " is a " + target.getClass().getName(), e);
        }
    }
    ...
    private void checkComplex(String attrName, Object target)
    {
        ...
        {
            throw new ResolverException("expected complex object but " + attrName + " is a " + Optional
                .ofNullable(target)
                .map(Object::getClass)
                .map(Class::getName)
                .orElse("null value"));
