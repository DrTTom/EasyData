...
/**
 * Repetitions are made by repeatedly resolving the content. Furthermore, this tag supports some data operations to
 * avoid extra pre-processing of data:
 * <ul>
...
public class ForTag extends ComplexTag
{
    ...
    public static final Pattern PATTERN = Pattern.compile(
        "FOR +(\\w+) *: *" + PATH_REGEX + "( +SELECT +" + PATH_REGEX + ")?( +UNIQUE)?( +(ASCENDING|DESCENDING)( +"
            + PATH_REGEX + ")?)? *");
    ...
    public void resolve(Token startTag, AccessibleData data, Writer output) throws IOException
    {
        try
        {
            ...
            {
                addressedCollection =
                    addressedCollection.substring(0, addressedCollection.length() - ".values".length());
