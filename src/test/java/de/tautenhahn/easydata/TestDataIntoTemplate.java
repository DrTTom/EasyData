...
/**
 * Creating some kind of document by inserting data into an appropriate template is a recurring task. There are many
 * implementations, for instance XSLT for XML, luatex for LaTeX, lots of frameworks for HTML. <br> Just how much
 * functionality do we need to create a useful document? What do we have to assume about the target language? How
 * complicated will it get?<br> This is a simple example which targets creation of arbitrary documents using data from a
 * JSON file.
 *
 * @author TT
 */
public class TestDataIntoTemplate extends DataIntoTemplateBase
{
    ...
    /**
     * Because there is no assumption made about the target document, we need to mark our special syntax with at least
     * one character which must be freely chosen in order not to interfere with the target syntax. Think about further
    ...
    /**
     * Template and document should be ideally handled streaming. First task is to expand some placeholder addressing a
     * single data value. For sake of simplicity, we treat a lists as maps with keys "0", "1" and so on. <br> Introduce
     * the [@=...] tag here.
     *
     * @throws IOException
     */
    ...
    /**
     * To freely access data, we need to use one expression to qualify another one. Use ${...} to de-reference inside an
     * expression. Do not worry about mixing up syntax, we are inside a marked tag here.
     *
     * @throws IOException
     */
    @Test
    ...
    /**
     * Need an iteration, in best case over keys and values of a map (or array treated as map). Furthermore, check
     * whether nested tags work.<br> Introducing tags [@FOR :] [@DELIM] [@END]
     */
    @Test
    ...
    /**
     * Need conditional parts. What kind of conditions do we need? At least an "==" operator is needed, assume there are
     * value literals supported as well. <br> Introducing tags [@IF ==] [@ELSE] [@END]
     */
    @Test
    public void conditional() throws IOException
    {
        ...
        result =
            doExpand("Der Exot ist [@FOR name:friends.keys][@IF friends.${name}.city==\"Rom\"][@=name][@END][@END].");
