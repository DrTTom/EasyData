...
class TestDataIntoTemplate {
    ...
    static void provideData() throws IOException {
        exampleData = DataTestHelper.getData("/data.json");
    }
    ...
    /**
     * We do not want many IF-statements asking x!=null, x!=0 or x!="". For these cases, a shorthand is provided.
     *
     * @throws IOException unexpected
     */
    @Test
    void conditionalWithoutOperator() throws IOException {
        for (String value : new String[]{"\"\"", "noSuchValue", "0", "false"}) {
            assertThat(doExpand(String.format("[@IF %s]present[@ELSE]missing[@END]", value)))
                    .as("value: " + value).isEqualTo("missing\n");
        }
        for (String value : new String[]{"\"something\"", "index", "1", "true"}) {
            assertThat(doExpand(String.format("[@IF %s]present[@ELSE]missing[@END]", value)))
                    .as("value: " + value).isEqualTo("present\n");
        }
    }

    @Test
    void shortHandIf() throws IOException {
        assertThat(doExpand("[@IF index]Index is [@=VALUE]![@END]"))
                .isEqualTo("Index is 2!\n");
    }


    private String doExpand(String template) throws IOException {
        return DataTestHelper.doExpand(template, exampleData, '[', MARKER, ']');
    }
}
